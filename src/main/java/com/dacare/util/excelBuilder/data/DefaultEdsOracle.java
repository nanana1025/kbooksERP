...
/**
 * EbDataSet 의 오라클 데이터베이스를 위한 페이징 데이터셋 구현 클래스
 *
 * <pre>
 *  ■ 설명
 *   - 오라클 데이터베이스를 쓸 경우 페이징이 필요할 때에만 사용
 ...
 * @version v2.0
 */
public abstract class DefaultEdsOracle implements EbDataSet {

	// 기본 프로퍼티
	public static final DbType dbType = EbDataSet.DbType.ORACLE;
	 ...
	 * @param pageSize
	 */
	public DefaultEdsOracle(UMap param, int pageSize) {
		dbParam = param;
	...
	public List<UMap> getListByPage(int page) {
		...
		dbParam.put("startNum", skip + 1);
		dbParam.put("endNum", skip + pageSize);
