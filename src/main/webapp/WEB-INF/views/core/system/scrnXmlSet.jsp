    ...
    <script>
        ...
        function gridbox(){
            ...
            var dataSource = new kendo.data.TreeListDataSource({
                transport: {
                    read: {
                    	 url: "/system/xmlList.json?xmlOwner=USER",
                         dataType: "json"
                    }
                },
                schema: {
...
// 		        { selectable: true },
        ...
        function doOnFormInit(name) {
            if (name == 'addXml') {
                ...
                location.href = '<c:url value="/system/xmlSave.do?owner=USER&xmlFolder="/>'+folder;
            } else if(name == 'delXml') {
                var grid = $('#gridbox').data('kendoTreeList');
                var selectedItem = grid.dataItem(grid.select());
                if(grid.select().length == 0) {
...
//                 var _xmlNm = selectedItem.SID;
                ...
                $.ajax({
                    ...
                    data:{xn:_xn, xmlTp: _xmlTp, xmlOwner: 'USER'},
...
//                 var _sid = selectedItem.SID;
                ...
                location.href = "<c:url value='/system/xmlEdit.do'/>" + "?xn="+ _xn + "&xmlTp="+_scrnTp +"&owner=USER";
            }
        }
