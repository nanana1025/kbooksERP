...
//                         	contentUrl  : "/menu.do?menu_sid=" + menu_sid + "&sid=" + "${sid}" + "&menuMode="+menuMode,
                ...
                $("#scrnArea").kendoSplitter({
                    ...
                    panes : [
                        {
                            ...
                            size		: ( area1_obj.size != undefined ) ? area1_obj.size : ""
                        },
                        {
                        	collapsible	: ( area1_obj.collapsible == 'Y' ) ? true : false,
                        	resizable	: ( area1_obj.resizable == 'Y' ) ? true : false
                    ...
                    contentLoad: function(e) {
                    	//console.log("area1_obj.height = "+area1_obj.height);
                    	if(scrnMode == 'vertical'){
                    		if(area1_obj.height){
                    			var scrnSplitter = $("#scrnArea").data('kendoSplitter');

                    			scrnSplitter.size("#pane1", area1_obj.height);
                    			scrnSplitter.resize();
                    		}
                       	}
                    	else{
                ...
                $("#subArea").kendoSplitter({
                    ...
                    contentLoad: function(e) {
                    	if(scrnMode == 'vertical'){
                    		...
                    		if(area3_obj.height){
                    			var subSplitter = $("#subArea").data('kendoSplitter');
                    			console.log("8888888888888888888888888 "+area3_obj.height);
//                     			subSplitter.size("#pane3", area3_obj.height);
                    			subSplitter.size("#pane3", "5%");
                    			subSplitter.resize();
                    		}
                       	}
                    	else{
                    		...
                    		if(area2_obj.width){
                    			var subSplitter = $("#subArea").data('kendoSplitter');
                    			subSplitter.size("#pane2", area2_obj.width);
                    			subSplitter.resize();
                    		}
                    		if(area3_obj.width){
                    			var subSplitter = $("#subArea").data('kendoSplitter');
                     			subSplitter.size("#pane3", area3_obj.width);
                    			subSplitter.resize();
                    		}
                    	}
                    	$(e.pane.children).ready(function(){
                ...
                subArea = $("#subArea").data('kendoSplitter');
            },
