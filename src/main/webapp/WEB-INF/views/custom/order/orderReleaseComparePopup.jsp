...
//  	$('#selectBtn').remove();
	...
	if('${sid}' == 'sale_order_customerpart_list'){
...
function fnGridInit(){
	...
	var openerGrid = opener.$('#sale_order_part_list_gridbox').data('kendoGrid');
	...
	var cobjectid = "ORDER_ID, COMPONENT_ID";
	var cobjectval = openerSelItem.order_id+','+openerSelItem.component_id;
	var qStr = "";
	var sid = '${sid}';
	var listNm = "";

	if('${sid}' == 'sale_order_customerpart_list')
		listNm ='sale_Order_CustomerPart_LIST';
	else
		listNm ='sale_Order_ReleasePart_LIST';
...
function fnReleaseReturnInventory() {
	...
	console.log("sale_order_releasepart_list.fnReleaseReturnInventory() Load");

	var url = '${releaseReturnInventory}';

	var grid = $('#sale_order_releasepart_list_gridbox').data('kendoGrid');
	var rows = grid.select();

	if(rows.length < 1) {GochigoAlert('선택된 항목이 없습니다.'); return;}

	var pGrid = opener.$('#sale_order_all_list_gridbox').data('kendoGrid');
