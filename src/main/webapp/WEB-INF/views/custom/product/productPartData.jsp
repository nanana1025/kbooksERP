...
// 	 	$('.k-button').css('float','right');
	 	...
	 	var openerGrid = opener.$('#admin_product_list_gridbox').data('kendoGrid');
...
// 			self.close();
		...
		$('#P_INVENTORY_ID').val(openerSelItem.p_inventory_id);
		$('#WAREHOUSING_ID').val(openerSelItem.warehousing_id);
...
// 	var pGrid = opener.$('#admin_product_list_gridbox').data('kendoGrid');
...
// 	if(!selItem) {GochigoAlert('선택된 제품이 없습니다.'); return;}
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.SUCCESS){
            	isSuccess = true;
                GochigoAlert(data.MSG, true, "DANGOL365");
            	//fnClose();
            }
            else{
