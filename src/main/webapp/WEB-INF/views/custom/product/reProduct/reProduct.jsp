...
// 										$('#PROXY_ID').val(data.PROXY_ID);
				    					 ...
				    					 GochigoAlertLocationHref("접수되었습니다. 전체 페이지로 이동합니다.", false, "dangol365 ERP", newUrl);
		...
		function addDetailSpec(productType, productSpec, componentId, componentCnt) {
			...
			var addItem =
					...
					productSpec +
					'</div>' +
					'<input type="number" id = "componentId-cnt-'+productType+'-'+componentId+'" class="view k-textbox col-input-number-short" value="'+componentCnt+'" min="0" max="999" onchange="countTotalProduct()">' +
...
// 		}
	...
	console.log("consigned_receipt.searchModel() Load");
	...
	item += '<td class="col-header-title" style="width: 80%;"> 모델명</td>';
			...
			success : function(data) {
				...
				if(searched_items.length > 0) {
					...
					for(var i=0; i<searched_items.length; i++) {
						...
						var modelId = searched_items[i].MODEL_LIST_ID;

						var item = '<tr>';
						item += '<td style="width: 10%;">' + (i+1) + '</td>';
						item +='<td style="width: 83%;">' + searched_items[i].MODEL_NM + '</td>';
						item += '<td style="width: 7%;"><button id="component-add-Model-'+modelId+'" class="k-button" onclick="addModel('+modelId+','+"'"+searched_items[i].MODEL_NM+"'"+')">담기</button>' ;
						item += '</tr>';
	...
	function addModel(modelListId, companyModelNm){

		console.log("modelListId = "+modelListId);
		console.log("companyModelNm = "+companyModelNm);
		var url = '${getModelListComponent}';
				...
				success : function(data) {
					...
					if(searched_items.length > 0) {
						...
						for(var i=0; i<searched_items.length; i++) {
							...
							var manufactureNm = searched_items[i].MANUFACTURE_NM;
							var modelNm = searched_items[i].MODEL_NM;
							var specNm = searched_items[i].SPEC_NM;
							var componentId = searched_items[i].COMPONENT_ID;
							var componentCnt = searched_items[i].COMPONENT_CNT;
							...
							addDetailSpec(productType, manufactureNm+" / "+modelNm+" / "+specNm, componentId, componentCnt)

						}
					}

					_modelId = modelListId;
			 		$('#selected-list-model-no-data').val(companyModelNm);
			 		$('#selected-list-model-no-data').text(companyModelNm);
				}
			});
...
// 			}).open('','',"top="+yPos+", left="+xPos+", width="+width+", height="+height);
			...
			// console.log('left: '+ ((window.screen.width/2) - (width/2)) + ', top: ' + ((window.screen.height/2) - (height/2)));
		...
		function fnFileUpload(){
			...
			var width, height;
			var url = '${consignedPopup}';
			var query = "?content=receiptFileupload"
			var width, height;
			width = "1600";
			height = "800";
			var xPos  = (document.body.clientWidth /2) - (width / 2);
			...
			window.open("<c:url value='"+url+query+"'/>", "receiptFileupload", "top="+yPos+", left="+xPos+", width="+width+", height="+height);
...
<body>
	...
	<div id="content" data-role="splitter" class="k-widget k-splitter content">
		...
		<div class="header_title">
			<span class="pagetitle"><span class="header-title30"></span> 생산대행 - 접수 </span>
		</div>


		<div id="consigned_receipt_btns" class="grid_btns" style="width: 100%">
<!-- 			<button onclick="fnOrderSheet()" class="k-button" style="float: left; width: 40px; background-color: slategray; margin-top:5px;margin-left:20px; padding: 3px 7px 3px 7px; font-size: 12px;"> -->
<!-- 				<span class="k-icon k-i-paste-plain-text" style="float: right; font-size: 18px; color: white;"></span> -->
<!-- 			</button> -->
			<button id="btnGoReceiptList" onclick="fnGoReceiptList()" class="k-button" style="float: left; width: 40px; background-color: slategray; margin-top:5px;margin-left:20px; padding: 3px 7px 3px 7px; font-size: 12px;">
...
<%--                                        <input type="input" class="view k-textbox col-input" id="RECEIPT_DT" name="RECEIPT_DT">--%>
                                        <input id="datetimepicker_acceptance_date" title="datetimepicker" style="width:100%; height: 25px; display: inline-block; margin-right: 3px; top: -1px;" readonly = "readonly"/>
                                    </td>
...
<%--                                        <input type="input" class="view k-textbox col-input" id="SALE_ROOT" name="SALE_ROOT" value="3">--%>
								...
								<tr>
									...
									<td class="col-content-25" style="height: 27px">
										<input type="radio" id="RELEASE_TYPE1" name="RELEASE_TYPE" value="1" checked /><label for="RELEASE_TYPE1">택배</label>
										<input type="radio" id="RELEASE_TYPE2" name="RELEASE_TYPE" value="2" /><label for="RELEASE_TYPE2">방문수령</label>
										<input type="radio" id="RELEASE_TYPE3" name="RELEASE_TYPE" value="3" /><label for="RELEASE_TYPE3">화물</label>
									</td>

								</tr>
								<tr>
									<td class="col-title">요청사항</td>
									<td class="col-content-25"><input type="input" class="view k-textbox col-input" style="width: 226%; height: 28px;"id="DES" name="ETC"></td>
								</tr>
				...
				<div class="scrollable-div-long">
					...
					<table class="stripe table_scroll">
						...
						<tr>
							...
							<td style="width: 10%">
								<button id="searchBtn_model" onclick="searchModel()" class="k-button col-search-button" data-role="button" role="button" aria-disabled="false" tabindex="1">찾기</button>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<button id="searchBtn_mon" onclick="searchItems('MON')" class="k-button col-search-button" data-role="button" role="button" aria-disabled="false" tabindex="1">찾기</button>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<button id="searchBtn_pkg" onclick="searchItems('PKG')" class="k-button col-search-button" data-role="button" role="button" aria-disabled="false" tabindex="1">찾기</button>
							</td>
						</tr>
