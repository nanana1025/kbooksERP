...
<html>
	...
	var _NAME = ["액정","배터리","마우스패드","키보드","CAM","USB","LAN_WIRELESS","LAN_WIRED","ODD","아답터","BIOS","OS"] ;
...
// 			contentType: "application/json",
	...
	function dataInitialize(data)
	{
		...
		if(data.EXIST && data.SUCCESS){
			...
			VARIABLE[1] = data.BATTERY * 1; //battery
			VARIABLE[2] = data.MOUSEPAD * 1; //mousePad
			VARIABLE[3] = data.KEYBOARD * 1; //keyboard
			VARIABLE[4] = data.CAM * 1; //cam
			VARIABLE[5] = data.USB * 1; //usb
			VARIABLE[6] = data.LAN_WIRELESS * 1; //lanWireless
			VARIABLE[7] = data.LAN_WIRED * 1; //lanWired
			hdd = data.HDD * 1; // hdd
			VARIABLE[8] = data.ODD * 1;	//odd
			VARIABLE[9] = data.ADAPTER * 1;	//adapter
			...
			VARIABLE[1] = 0; //battery
			VARIABLE[2] = 0; //mousePad
			VARIABLE[3] = 0; //keyboard
			VARIABLE[4] = 0; //cam
			VARIABLE[5] = 0; //usb
			VARIABLE[6] = 0; //lanWireless
			VARIABLE[7] = 0; //lanWired
			hdd = 0; // hdd
			VARIABLE[8] = 0;	//odd
			VARIABLE[9] = 0;	//adapter
			VARIABLE[10] = 0;	//bios
			VARIABLE[11] = 0;	//os
		}
        ...
        for(var i = 0 ; i < 5; i++) {
            ...
            if((hdd & _BCASE[i]) == _BCASE[i]) {
                ...
                if( !$(".step10").hasClass("changed") ) {
                    $(".step10").addClass("changed");
                }
            }
        }
...
<table class="tg">
    ...
    <tbody>
    <tr>
        <td class="tg-yjjc" rowspan="14">NTB</td>
        ...
        <td class="tg-buh4" onclick="checkCaseChanged()">
        	...
        	<tr>
	        	...
	        	<td class="tg-buh4  tg-shlee">
		        	 <form>힌지파손
	                	<input type="checkbox" name="힌지파손" value="1" />
	            	</form>
	        	</td>
        	</tr>
        	</table>
    ...
    <tr>
        ...
        <td class="tg-buh4 step3">배터리</td>
        <td class="tg-buh4" onclick="checkRadioChanged('.step3', '배터리')">
            <form>
                <input type="radio" name="배터리" value="0" />해당없음
<!--                 <input type="radio" name="배터리" value="0" />인식 안 됨 -->
                <input type="radio" name="배터리" value="1" />충전 안 됨
                <input type="radio" name="배터리" value="2" />40% 이하
                <input type="radio" name="배터리" value="3" />80% 이하
                <input type="radio" name="배터리" value="4" />고정락 불량
            </form>
        </td>
    </tr>
    <tr>
        ...
        <td class="tg-0lax" onclick="checkRadioChanged('.step4', '마우스패드')">
            <form>
                <input type="radio" name="마우스패드" value="0" />해당없음
                <input type="radio" name="마우스패드" value="1" />인식 안 됨
                <input type="radio" name="마우스패드" value="2" />스크래치
                <input type="radio" name="마우스패드" value="3" />좌우 버튼 인식 안 됨
            </form>
        </td>
    </tr>
    <tr>
        ...
        <td class="tg-buh4" onclick="checkRadioChanged('.step5', '키보드')">
            <form>
                <input type="radio" name="키보드" value="0" />해당없음
                <input type="radio" name="키보드" value="1" />인식 안 됨
                <input type="radio" name="키보드" value="2" />자판빠짐 1개
                <input type="radio" name="키보드" value="3" />자판빠짐 2개
                <input type="radio" name="키보드" value="4" />자판빠짐 3개 이상
            </form>
        </td>
    </tr>
    <tr>
        <td class="tg-0lax step6">6단계</td>
        <td class="tg-0lax step6">CAM</td>
        <td class="tg-0lax" onclick="checkRadioChanged('.step6', 'CAM')">
            <form>
    ...
    <tr>
        ...
        <td class="tg-buh4 step7">USB</td>
        <td class="tg-buh4" onclick="checkRadioChanged('.step7', 'USB')">
            <form>
    ...
    <tr>
        ...
        <td class="tg-0lax step8">LAN[무선]</td>
        <td class="tg-0lax" onclick="checkRadioChanged('.step8', 'LAN_WIRELESS')">
            <form>
    ...
    <tr>
        ...
        <td class="tg-buh4 step9">LAN[유선]</td>
        <td class="tg-buh4" onclick="checkRadioChanged('.step9', 'LAN_WIRED')">
            <form>
    ...
    <tr>
        ...
        <td class="tg-0lax step10">HDD</td>
    ...
    <tr>
        ...
        <td class="tg-buh4 step11">ODD</td>
        <td class="tg-buh4" onclick="checkRadioChanged('.step11', 'ODD')">
            <form>
    ...
    <tr>
        ...
        <td class="tg-0lax step12">아답터</td>
        <td class="tg-0lax" onclick="checkRadioChanged('.step12', '아답터')">
            <form>
    ...
    <tr>
        <td class="tg-buh4 step13">13단계</td>
        <td class="tg-buh4 step13">BIOS</td>
        <td class="tg-buh4" onclick="checkRadioChanged('.step13', 'BIOS')">
            <form>
    ...
    <tr>
        ...
        <td class="tg-0lax step14">윈도우</td>
        <td class="tg-0lax" onclick="checkRadioChanged('.step14', 'OS')">
            <form>
