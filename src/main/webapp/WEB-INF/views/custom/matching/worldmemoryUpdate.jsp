...
$(document).ready(function() {
	...
	//$('#saveBtn_admin_component_matching_cpu_list').remove();
	//$('#cancelBtn_admin_component_matching_cpu_list').remove();


	setTimeout(function() {
		window.resizeTo(784, 320);
...
function fnCancle(){
	console.log("load [fnCancle.js]");
	 ...
	 $('#cancelbtn_admin_component_matching_cpu_list').kendoButton();
     var cancelBtn = $('#cancelbtn_admin_component_matching_cpu_list').data('kendoButton');
...
function SetInitDate(){
	...
	var grid = opener.$('#admin_component_matching_cpu_list_gridbox').data('kendoGrid');
	var selItem = grid.dataItem(grid.select());
	var params = {
			MODEL_NM : selItem.model_nm,
			PRICE_ID : selItem.price_id
	}
