...
$(document).ready(function() {
	...
	var infCondGetComponent = '<button id="${sid}_GetComponentBtn" type="button" onclick="fnGetComponent()" class="k-button" style="float: right;">부품 추가</button>';
	...
	var infcondComponentAll = infCondInsertComponent +  infCondGetComponent;
...
//     var infCondCheck = '<button id="${sid}_check_btn" type="button" onclick="fnInventoryCheck()" class="k-button" style="float: right;">검수</button>';
	...
	$('#${sid}_btns').prepend(infCondComponent);
	$('#${sid}_btns').prepend(infCondCompanyComponent);
	$('#${sid}_btns').prepend(infCondComponentUpdateBulk);
...
function prohibit(){
	...
	var pGrid = $('#warehousing_consigned_all_list_gridbox').data('kendoGrid');
...
function fnComponentUpdateBulk(){
	...
	console.log( '${ptype}');
	var pid = 'warehousing_consigned_examine_list';
	var ptype = '${ptype}';
...
function fnConsignedCompanyInventoryList(){
	...
	var url = "&pid=${sid}&CUSTOMKEY=WAREHOUSING_ID,COMPONENT_ID,INVENTORY_TYPE&CUSTOMVALUE="+params.WAREHOUSING_ID+","+params.COMPONENT_ID+",N";
