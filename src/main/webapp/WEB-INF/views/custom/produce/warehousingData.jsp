...
$(document).ready(function() {
	console.log("load [warehousingData.jsp]");

    $('#saveBtn_${sid}').remove();

 	setTimeout(function() {
        var infCond = '<button id="insert_admin_produce_warehousing_data" onclick="fnUpdateWarehousing()" class="k-button" data-role="button" role="button" aria-disabled="false" tabindex="1">수정</button>';
...
function fnUpdateWarehousing() {
	console.log("${sid}.fnUpdateWarehousing() Load");
		...
		actions: [{
			...
			action: function(e){
				...
				if(isSuccess){
					...
					if(sid ==  "admin_produce_warehousing_data")
						fnObj('LIST_admin_produce_warehousing_all_list').reloadGrid();
					else
						fnObj('LIST_warehousing_consigned_all_list').reloadGrid();
				}
...
function setWarehousing(params){
	...
	console.log("${sid}.setWarehousing() Load");

	var sid = "${sid}";
	var pid;
	if(sid ==  "admin_produce_warehousing_data")
		pid = "admin_produce_warehousing_all_list";
	else
		pid = "warehousing_consigned_all_list";
