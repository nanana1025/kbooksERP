...
$(document).ready(function() {
	...
	console.log("load [inventoryAllListForProducePartData.js]");

	//$('.basicBtn').remove();
	//$('#selectBtn').remove();
	$('#produce_part_inventory_list_gridbox').off('dblclick');
...
function fnCustomSelect(){
	...
	var grid = $('#produce_part_inventory_list_gridbox').data('kendoGrid');
	var selItem = grid.dataItem(grid.select());
	if(!selItem) {GochigoAlert('선택된 항목이 없습니다.');
	return;}


	var openerGrid = opener.opener.$('#admin_inventory_all_list_gridbox').data('kendoGrid');
	var openerSelItem = openerGrid.dataItem(openerGrid.select());
	if(!openerSelItem) {GochigoAlert('부모의 선택된 항목이 없습니다.');
	return;}
	...
	opener.$('#INVENTORY_ID').val(openerSelItem.inventory_id);
	opener.$('#C_INVENTORY_ID').val(params.INVENTORY_ID);
