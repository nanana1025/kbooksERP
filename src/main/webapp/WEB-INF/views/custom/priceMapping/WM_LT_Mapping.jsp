...
<c:url var="updateCustomMappingCancel"					value="/priceMapping/updateCustomMappingCancel.json" />
...
$(document).ready(function() {
	...
	console.log("load [WM_LT_Mapping.js]");
...
function fnAddLTCustom(){
	console.log("${sid}.fnAddLTCustom() Load");

	var width = 800;
	var height = 450;

	var grid = $('#pricemapping_wm_lt_list_gridbox').data('kendoGrid');
	var rows = grid.select();
	if(rows.length < 1) {GochigoAlert('선택된 항목이 없습니다.'); return;}

	var xn = "priceMapping_WM_LT_LIST";

	var pid = 'pricemapping_wm_lt_list';
	var ptype = '${ptype}';
...
function fnDeleteLTCustom(){
	console.log("${sid}.fnDeleteLTCustom() Load");

	var url = '${updateCustomMappingCancel}';
	...
	$("<div></div>").kendoConfirm({
	    ...
	    content: "<h3>선택한 부품의 매핑을 취소하시겠습니까?</h3>"
...
function fnPriceConfirm() {
	...
	$("<div></div>").kendoConfirm({
		...
		minWidth : 200,
		title: fnGetSystemName(),
	    content: "<h3>선택한 부품의 가격 매칭 정보를 확정하시겠습니까?</h3><br><b><font color=blue>* 매칭을 확정하면 선택한 부품의 가격이 '현재 W사 가격'으로 저장됩니다.</font>"
	}).data("kendoConfirm").open();
	//끝지점
}
...
function fnDropBoxonChange(page, pageSize){
	...
	var grid = $('#pricemapping_wm_lt_list_gridbox').data('kendoGrid');
	 var selItem = grid.dataItem(grid.select());
     var keyValue = selItem.other_purchase_part_id;
	...
	var qStr = "";
   	...
   	if(check == "ALL"){
   		qStr += '&cobjectid= OTHER_PURCHASE_PART_ID';
	    qStr += '&cobjectval=' + keyValue;
	}else{
		qStr += '&cobjectid= OTHER_PURCHASE_PART_ID,' + cobjectid;
	    qStr += '&cobjectval= ' +keyValue +','+  check;
	}
