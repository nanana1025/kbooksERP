<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<c:url var="urlOfferVoucherJson"	value="/charging/offerVoucher.json" />

<!DOCTYPE html>
<html>
<head>
<title>이용권 제공</title>
<meta http-equiv="X-UA-Compatible" content="IE=edge, chrome=1" />
<meta charset="utf-8"/>

<link rel="stylesheet" type="text/css" href="/codebase/css/examples-offline.css"/>
<link rel="stylesheet" type="text/css" href="/codebase/kendo/css/kendo.common.min.css"/>
<link rel="stylesheet" type="text/css" href="/codebase/css/kendo.custom.css"/>
<link rel="stylesheet" type="text/css" href="/codebase/style.css"/>
<script src="/js/jquery-1.12.4.js"></script>
<script src="/codebase/kendo/kendo.all.min.js"></script>
<script src="/codebase/kendo/cultures/kendo.culture.ko-KR.min.js"></script>
<script src="/codebase/kendo/messages/kendo.messages.ko-KR.min.js"></script>
<script src="/codebase/common.js"></script>
<script src="/codebase/default.js"></script>
<script src="/codebase/gochigo.kendo.common.js"></script>
<script src="/codebase/gochigo.kendo.util.js"></script>
<script src="/codebase/gochigo.kendo.util.js"></script>

<style type="text/css">
#popContainer {
	background-color: #eff2f7;
	width: 100%;
	height: 100%;
}
.fieldSet {
	display: inline-block;
	margin: 10px 10px;
	background-color: #ffffff;
	border: 1px solid #d6d6d9;
}
</style>
<script type="text/javascript">
	$(document).ready(function() {
		fnSetUserNmAndEmail();

		$("#NURSE_CNT_CD").kendoDropDownList();
		$("#VOUCHER_TYPE_CD").kendoDropDownList();

 		$('#VOUCHER_CNT').on('keyup', function(e) {
 			removeChar(e);
 		});
    });

	function fnSetUserNmAndEmail() {
		var userNmStr = opener._userNms.join(',');
		$('#USER_NM').val(userNmStr);

		var emailStr = opener._emails.join(',');
		$('#LOGIN_ID').val(emailStr);
	}

	function fnOfferVoucher() {
		if(!$('#VOUCHER_CNT').val()) {
			alert('개수는 필수 입력입니다.');
			return;
		}

		var url = '${urlOfferVoucherJson}';
		var params = {
			MATRON_ID: opener._matronIds.join(','),
			NURSE_CNT_CD: $('#NURSE_CNT_CD').val(),
			VOUCHER_TYPE_CD: $('#VOUCHER_TYPE_CD').val(),
			VOUCHER_CNT: $('#VOUCHER_CNT').val(),
			NOTE: $('#NOTE').val()
		};

		$.post(url, params, function(data) {
			if(data) {
				if(data.success) {
					alert('이용권이 제공됐습니다.');
					opener.fnReloadGrid();
					fnClose();
				} else {
					alert('이용권 제공이 실패했습니다.');
				}
			}
		})
		.fail(function(data) {
			alert(data.responseJSON.errMsg);
		});
	}

	function fnClose() {
		self.close();
	}
</script>
</head>
<body>
<div id="popContainer">
	<div id="partial_cancellation_header_title" class="header_title">
		<span class="pagetitle"><span class="k-icon k-i-copy"></span>이용권 제공</span>
	</div>
	<div class="content">
		<div id="partial_cancellation_view-btns" class="view_btns">
			<button id="saveBtn" class="k-button" onclick="fnOfferVoucher();">이용권 제공</button>
			<button id="cancelBtn" class="k-button" onclick="fnClose();">취소</button>
		</div>
		<fieldSet class="fieldSet">
			<input type="hidden" id="FILE_COL" name="FILE_COL" value=""/>
			<table id="cancel-amt-table">
				<colgroup width="800px">
					<col width="150px"/>
					<col width="*"/>
				</colgroup>
				<tbody>
					<tr>
						<td style="text-align: right;">사용자명</td>
						<td><input type="text" id="USER_NM" class="USER_NM k-textbox" style="width: 100%;" value="" disabled="disabled" /></td>
					</tr>
					<tr>
						<td style="text-align: right;">이메일</td>
						<td><input type="text" id="LOGIN_ID" class="LOGIN_ID k-textbox" style="width: 100%;" value="" disabled="disabled" /></td>
					</tr>
					<tr>
						<td style="text-align: right;"><span class="requiredDot">* </span>간호사수</td>
						<td>
							<select id="NURSE_CNT_CD" name="NURSE_CNT_CD" style="width: 100%;">
								<c:forEach var="nurse" items="${nurseCntCd}" varStatus="status">
									<option value="${nurse.CODE_CD}">${nurse.CODE_NM}</option>
								</c:forEach>
							</select>
						</td>
					</tr>
					<tr>
						<td style="text-align: right;"><span class="requiredDot">* </span>이용권 종류</td>
						<td>
							<select id="VOUCHER_TYPE_CD" name="VOUCHER_TYPE_CD" style="width: 100%;">
								<c:forEach var="voucher" items="${voucherTypeCd}" varStatus="status">
									<option value="${voucher.CODE_CD}">${voucher.CODE_NM}</option>
								</c:forEach>
							</select>
						</td>
					</tr>
					<tr>
						<td style="text-align: right;"><span class="requiredDot">* </span>개수(숫자만 입력)</td>
						<td><input type="text" id="VOUCHER_CNT" class="VOUCHER_CNT k-textbox" style="width: 100%;" value="" /></td>
					</tr>
					<tr>
						<td style="text-align: right;">비고</td>
						<td><textarea id="NOTE" class="NOTE k-textbox" cols="100" rows="5" style="width: 100%; resize: none;"></textarea></td>
					</tr>
				</tbody>
			</table>
		</fieldSet>
	</div>
</div>
</body>
</html>
