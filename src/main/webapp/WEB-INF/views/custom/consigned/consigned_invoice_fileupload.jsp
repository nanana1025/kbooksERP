...
<c:url var="invoiceConsignedByExcel"					value="/consigned/invoiceConsignedByExcel.json" />


<head>

	<meta http-equiv="X-UA-Compatible" content="IE=edge, chrome=1" />
  	<title>DANGOL365 ERP | 송장 업로드</title>
	...
	<script type="text/javascript">
		...
		var _COLUMN_NM = ["접수번호", "고객명", "배송업체", "송장번호"];
		var _CO_NM = ["RECEIPT", "CUSTOMER_NM", "DELIVERY_COMPANY", "INVOICE"];
...
// 			console.log("파일_업로드_임시.jsp");
	...
	function fnGoReceiptList()
	{
		...
		function BindTable(jsondata, tableid) {/*Function used to convert the JSON array to Html Table*/
			...
			console.log("fileColumnSet.length = "+fileColumnSet.length);
...
// 						console.log("key = "+key)
	...
	<script>
		...
		function fnFileUpload() {
			...
			console.log("${sid}.fnFileUpload() Load");
			...
			var url = '${invoiceConsignedByExcel}';
			var params = {
// 					DATA_CNT: _excelData.length
			};
			...
			for(var i=0; i<_excelData.length; i++){
					...
					for(var j = 0; j <_COLUMN_NM.length; j++ )
							subParams[_CO_NM[j]] = _excelData[i][_COLUMN_NM[j]];

					params["DATA"+i] = subParams;
			}
	    			...
	    			action: function(e){
				    	...
				    	$.ajax({
				    		...
				    		success : function(data) {
				    			...
				    			if(data.SUCCESS)
				    			{
				    				opener.fnObj('LIST_consigned_all_invoice_list').reloadGrid();
