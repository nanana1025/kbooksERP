...
$(document).ready(function() {
	...
	console.log("load [consignedInvoiceAll.js]");
	// $('.basicBtn').remove();
	...
	<div style="border: 0px solid gold; float: left; width: 15%;">\
 	...
 	$('#${sid}_uploadBtn').css('float', 'right');
	$('#${sid}_uploadBtn').css('margin-top', '35px');
    $('#${sid}_printBtn').css('margin-top', '35px');
    $('#${sid}_printBtn').css('float', 'right');
    $('#${sid}_printBtn').text("리스트 출력")
...
function fnDropBoxonChange(){
 ...
 function fnUploadInvoice(){

	 console.log("${sid}.fnUploadInvoice() Load");

	 var width, height;
		var url = '${consignedPopup}';
		var query = "?content=invoiceFileupload"
		var width, height;
		width = "800";
		height = "800";
		...
		window.open("<c:url value='"+url+query+"'/>", "invoiceFileupload", "top="+yPos+", left="+xPos+", width="+width+", height="+height);
