...
$(document).ready(function() {
	...
	console.log("load [consignedProxyAllList.js]");
	$('.basicBtn').remove();
// 	$('#consigned_proxy_all_list_gridbox').off('dblclick');

	var infCond = '<label>입고상태 &nbsp;&nbsp;&nbsp;</label><input id="warehousing_state" style = "width: 150px;"/>';

    $('#consigned_proxy_all_list_btns').prepend(infCond);
...
function fnDropBoxonChange(){
	...
	console.log("consignedProxyAllList.fnDropBoxonChange() Load");
	...
	var dataSource = {
           transport: {
               read: {
            	   url: "<c:url value='"+url+"'/>"+"?xn=consigned_proxy_all_list"+"&"+qStr,
                    dataType: "json"
               },
	...
	var grid = $("#consigned_proxy_all_list_gridbox").data("kendoGrid");
