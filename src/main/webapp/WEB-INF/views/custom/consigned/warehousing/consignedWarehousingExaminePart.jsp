...
$(document).ready(function() {
	...
	console.log("load [ConsignedWarehousingExaminePart.jsp]");

 	$('#saveBtn_consigned_warehousing_examine_list').remove();
 	...
 	setTimeout(function() {
	 	...
	 	if(ID == null || ID == "")
	 		infCond = '<button id="insert_consigned_warehousing_examine_list" onclick="fnInsertInfo()" class="k-button" data-role="button" role="button" aria-disabled="false" tabindex="1">저장</button>';
	 	else{
 	 		$('#BARCODE').attr("readonly", "readonly");
// 	 		$('#BARCODE').attr("disable", "disable");

	 		$('.k-i-windows').attr("onclick", null);
	 		infCond = '<button id="update_consigned_warehousing_examine_list" onclick="fnUpdateInfo()" class="k-button" data-role="button" role="button" aria-disabled="false" tabindex="1">수정</button>';
	 	}

	 	$('#consigned_warehousing_examine_list_view-btns').prepend(infCond);
...
function prohibit(){
	...
	var pGrid = opener.$('#consigned_warehousing_all_list_gridbox').data('kendoGrid');
...
function fnInsertInfo() {
	...
	console.log("consigned_warehousing_examine_list.fnInsertInfo() Load");
	...
	if(isSuccess){
		opener.fnObj('LIST_consigned_warehousing_all_list').reloadGrid();
...
// 	여기에 입력
    ...
    if(isSuccess){
    	opener.fnObj('LIST_consigned_warehousing_all_list').reloadGrid();
//     	opener.fnObj('LIST_consigned_warehousing_all_list').reloadGridCustom(queryCustom);
		//fnClose();
	}
}
