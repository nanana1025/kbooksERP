...
<c:url var="receiptConsignedByExcel"					value="/consigned/receiptConsignedByExcel.json" />


<head>

	<meta http-equiv="X-UA-Compatible" content="IE=edge, chrome=1" />
  	<title>DANGOL365 ERP | 접수 업로드</title>
	...
	<script type="text/javascript">
		...
		var _COLUMN_NM = ["제품유형", "보증기간", "고객명1", "고객휴대폰1", "고객전화번호1", "고객명2", "고객휴대폰2","고객전화번호2", "우편번호","고객주소","모델","CPU", "MBD","MEM1","MEM2","SSD","HDD","VGA","모니터","파워", "키보드", "마우스", "케이블", "주변기기","OS", "쿠폰", "AIR", "박스", "요청사항" ];
		var _CO_NM = ["PC_TYPE", "GUARANTEE_DUE", "CUSTOMER_NM_S", "MOBILE_S", "TEL_S", "CUSTOMER_NM_R", "MOBILE_R","TEL_R", "POSTAL_CD","ADDRESS","MODEL_NM","CPU", "MBD", "MEM1","MEM2", "SSD","HDD", "VGA", "MON", "POW", "KEY" ,"MOU","CAB","PER", "OS","COUPON", "AIR", "BOX", "DES" ];
...
// 			console.log("파일_업로드_임시.jsp");
            ...
            for(var i = 0 ; i< _COLUMN_NM.length; i++)
				fileColumnSet.push(_COLUMN_NM[i]);
...
//
	...
	<script>
		...
		function fnFileUpload() {
			...
			if(rowLength < 2){
				GochigoAlert("데이터가 없습니다.");
				return;
			}

			var keyName = '';

			var url = '${receiptConsignedByExcel}';
...
// 	    							params[_CO_NM[y-1]] = data;
// 	    						}
						for(var i=0; i<_excelData.length; i++){
								var row = table.rows[i+1];
 	    						var params = {};

								for(var j = 0; j <_COLUMN_NM.length; j++ )
									params[_CO_NM[j]] = _excelData[i][_COLUMN_NM[j]];
						    	...
						    	$.ajax({
						    		...
						    		success : function(data) {
						    			...
						    			if(data.SUCCESS)
						    			 	row.cells[0].textContent = data.RECEIPT;
						    			else
					    					row.cells[0].textContent = data.MSG;
						    		}
						    	});
		    				}

	    				GochigoAlert("접수되었습니다.");
	    			}
	    		},
	    		{
	    			text: '닫기'
...
<body>
	...
	<div>
		...
		<div class="grid_btns" style="width: 100%; margin: 10px;">
			<button id="consigned_receipt_close_btn" type="button" onclick="fnGoReceiptList()" class="k-button" style="float: right;">목록으로이동</button>
		...
		<div class="main">
			...
			<div style="text-align: center;">
				...
				<div class="view-table" style="text-align: left; margin-left: 30px; display: none;">
					...
					<div class="scrollable-div" style="width: 96.5%; margin-top: 10px; overflow-x: auto; overflow-y: auto;">
						<table id="excel-table" class="stripe" style="width: 100%;">
						</table>
					</div>
<!-- 					<div class="scrollable-div" style="width: 99%; height: 300px; overflow-x: auto; overflow-y: auto;"> -->
<!-- 						<table id="excel-table" class="stripe" style="width: 99%;"> -->
<!-- 						</table> -->
<!-- 					</div> -->
