...
$(document).ready(function() {
	...
	console.log("load [CreateConsignedCoreComponent.jsp]");
...
function fnComponentInsert() {
	...
	if($('#RELEASES').val().length != 10){
		GochigoAlert('올바르지 않은 출고번호 입니다.');
		return;
	}
...
// 			USER_ID: 'shlee'
	...
	if(_componentCd == 'CPU'){
		...
		params.SB_NM = $('#COL2').val();
		params.MBD_MODEL_NM = $('#COL3').val();
		params.PRODUCT_NAME = $('#COL3').val();
	}else if(_componentCd == 'MEM'){
		params.CAPACITY = ($('#COL4').val()*1024)+"MBytes";
		params.MODULE_NM = $('#COL2').val();
		params.BANDWIDTH = $('#COL3').val();
	}else if(_componentCd == 'STG'){
		params.CAPACITY = $('#COL3').val();
		params.STG_TYPE = $('#STG_TYPE').val();
